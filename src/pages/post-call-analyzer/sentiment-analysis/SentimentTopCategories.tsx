...
interface SentimentTopCategoriesProps {
    ...
    <div className="bg-popover/95 backdrop-blur-sm border border-border rounded-lg p-3 shadow-xl min-w-[140px]">
      <div className="space-y-1.5">
        {payload.map((item: any, index: number) => {
          ...
          const name = item.payload?.name || item.name || "Category"; // Get name from payload
...
export const SentimentTopCategories = ({ 
  selectedSentiment, 
  onCategorySelect,
  onClose,
  dateRange,
    ...
    const loadCategoryData = async () => {
      ...
      try {
        ...
        const filters = {
          ...
          sentiment_type: selectedSentiment.toLowerCase()
        };

        const response = await callRoutingApiService.UserSentimentCategory(filters);
        ...
        console.error('Error loading sentiment categories:', error);
        setError(true);
        setCategoryData([]);
      } finally {
        setLoading(false);
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-center h-[400px]">
            <ExceptionHandleView 
              ...
              content="sentiment category data"
              onTryAgain={onRetry}
            />
          </div>
        </CardContent>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-center h-[400px]">
            <ExceptionHandleView 
              ...
              content={`${selectedSentiment} sentiment categories for the selected period`}
              onTryAgain={onRetry}
            />
          </div>
        </CardContent>
